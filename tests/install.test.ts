...
import {ILoggerOptions} from "../src/blue-logger/interfaces/logger-options";
const expect = chai.expect;

describe("blue-logger.ts", () => {
    ...
    test("install() should throw an error with the an incorrect parameter.", () => {
        ...
        expect(() => {
            BlueLogger.install(Vue, options);
        })
            .to
            .throw(Error, "Provided options for blue-logger are not valid.");
    });
});
